...
router.post("/", auth.required, function (req, res, next) {
  User.findById(req.payload.id)
    .then(async function (user) {
      ...
      if (!item?.image) {
        try {
          const openAiImage = await openAI.createImage({
            prompt: item?.title || "a cute baby kitten",
            n: 1,
            size: "256x256",
          });
          item.image = openAiImage.data.data[0].url;
        } catch (e) {
          console.log(e);
          item.image =
            "https://openai-labs-public-images-prod.azureedge.net/user-BfIQYxoMCdlROP7TbgIq1nOC/generations/generation-o5gQcDiH6ov91DEzPxLVQjmE/image.webp";
        }
      }
